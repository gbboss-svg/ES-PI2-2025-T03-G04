document.addEventListener("DOMContentLoaded", () => {
  ...
  function isCPF(cpf) {
    ...
    let soma = 0,
      resto
    for (let i = 1; i <= 9; i++) soma += Number.parseInt(cpfLimpo.substring(i - 1, i), 10) * (11 - i)
    resto = (soma * 10) % 11
    if (resto === 10 || resto === 11) resto = 0
    if (resto !== Number.parseInt(cpfLimpo.substring(9, 10), 10)) return false
    soma = 0
    for (let i = 1; i <= 10; i++) soma += Number.parseInt(cpfLimpo.substring(i - 1, i), 10) * (12 - i)
    resto = (soma * 10) % 11
    if (resto === 10 || resto === 11) resto = 0
    if (resto !== Number.parseInt(cpfLimpo.substring(10, 11), 10)) return false
    return true
  }
  ...
  function isTelefoneComDDI(telefone) {
    ...
    if (t.startsWith("55")) {
      ...
      if (numero.length === 9) {
        ...
        const primeiro = Number.parseInt(numero.charAt(0), 10)
  ...
  identifierInput.addEventListener("input", (e) => {
    ...
    if (value.trim().startsWith("+") || numbers.startsWith("55")) {
      if (numbers.startsWith("55")) {
        ...
        if (rest.length <= 11) {
          if (rest.length === 11)
            e.target.value = `+55 (${rest.substring(0, 2)}) ${rest.substring(2, 7)}-${rest.substring(7)}`
          else if (rest.length === 10)
            e.target.value = `+55 (${rest.substring(0, 2)}) ${rest.substring(2, 6)}-${rest.substring(6)}`
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (
      tipo === "Email-Invalido" ||
      tipo === "Telefone-Invalido" ||
      tipo === "Indefinido" ||
      tipo === "Possivel-Telefone-Sem-DDI"
    ) {
